...
# # Create EC2 instance
resource "aws_instance" "main" {           # 1 ses-46
    ami = local.ami_id
...
# ec2 server or instance configuring here 
resource "terraform_data" "main" {             # 2 sess-46
  triggers_replace = [
...
# Stopping the serevr or instance 
resource "aws_ec2_instance_state" "main" {              #3 sess-46
...
# Take AMI
resource "aws_ami_from_instance" "main" {                  # 4 sess-46
...
# creating TG
resource "aws_lb_target_group" "main" {               # 5 sess-46
...
# creating LUNCH template 
resource "aws_launch_template" "main" {             # 6 sess-46
  name = "${local.common_name_suffix}-${var.component}"
...
# ASG created
resource "aws_autoscaling_group" "main" {           # 7 sess-46
  name                      = "${local.common_name_suffix}-${var.component}"
  max_size                  = 10
  min_size                  = 1
...
# ASG POLICY Created
resource "aws_autoscaling_policy" "main" {          # 8 sess-46
...
# Listener Created 
resource "aws_lb_listener_rule" "main" {    # 9 sess-46
...
# Deleted Instance or server
resource "terraform_data" "main_local" {                # 10 sess-46
  triggers_replace = [
    aws_instance.main.id
  ]
